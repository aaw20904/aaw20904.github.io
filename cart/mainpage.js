...
function Modell(storageKey){
    ...
    function getCartFromStorage (keyID) {
            ...
            var jsonRaw;
            var complexObj;
            /*read JSON from the storage*/
            jsonRaw = cartStorage.getItem(keyID);
            /*parses a string for getting a full object*/
            complexObj = JSON.parse(jsonRaw);
...
/* MessageDispatcherFromItems */
function MessageDispatcherFromItems (itemsWrapperID) {
    /*an array with nodes of all the buttons 'add to Cart'
    that helds "goods container".*/
    var buttonNodes = null;
    /*it is an interface to USE CASE interactor.
    This function will be called by a click event
     handler-function*/
    var interfaceToUseCase = null;
    var goodsContainer = document.getElementById(itemsWrapperID);
    /*checking- if the wrapper exists*/
    if (!goodsContainer) {
        return null;
    }
    /*this function makes an array of nodes-buttons
      These buttons must be into the 'goods container'*/
     function makeButtonsArray (parentID) {
        ...
        var btnArray;
         ...
         htmlCollection = pNode.getElementsByTagName('button');
         /*converting to an array;*/
         btnArray =  Array.prototype.slice.call(htmlCollection);
         return btnArray; 
     }
    
    /*An event handler - this func. will be bound to a button event*/
    function clickEventHandler (event) {
        var composed = event.composedPath();
        var idOfGood = composed[0].attributes['data-btn-good-id'].nodeValue;
        if (interfaceToUseCase) {
           interfaceToUseCase(idOfGood);
        }
    }
    
    return {
        registerCallback: function (f) {
            interfaceToUseCase = f;
        },
        bindButtons: function () {
            if (buttonNodes) {
                return null;
            }
            /*get an array of buttons (next to each item)*/
            buttonNodes = makeButtonsArray('buttonsAddHolder');
            /*iterate the Array and bind an EventHandler to buttons*/
            buttonNodes.forEach (function(z) {
                z.addEventListener('click',clickEventHandler,false);
            } );
        },
        unbindButtons: function () {
            /*get an array of buttons (next to each item)*/
            buttonNodes = makeButtonsArray('buttonsAddHolder');
            /*iterate the Array and unbind an EventHandler to buttons*/
            buttonNodes.forEach (function(z) {
                z.removeEventListener('click',clickEventHandler);
            } );
        }
    }
}

/*A GoodsItemFabrica class********************/
/* { key:{name:``, price:``, ... image:{srcset:"",sizes:"",src:""} } }*/
function GoodsItemsFabrica (itemsWrapperID) {
    var parentNode = document.getElementById(itemsWrapperID);
    if (!parentNode) {
        return null;
    }
    return {
     removeItem: function (key) {
         /*get a node*/
         var childNode = parentNode.querySelector('[data-item-presenter-key=' + key + ']');
         /*remove child node - if it is exists*/
         if (childNode) {
           parentNode.removeChild(childNode);
         }
         
     },
     addItem: function (data,templ = null) {
         var tmp;
         var tmpSubnode;
         ...
         /*if a template isn`t exists*/
           if (!templ){
             templ =   "<img data-item-field-id='image'/>"+
                  ...
                  "<button data-btn-good-id='price' class='addToCartBtn'>Add to cart</button>";
           }
         ...
         /*aply a new node to a parent node*/
...
/*************************/
/**for smooth scrolling v1 --- experimental, not use*/
function ScrollCartByStep () {
    var nodeContainer;
    var currentIndex = 0;
    function onScroll (evt) {
        /*get a HTML collection*/
        var childrenArray = evt.currentTarget.querySelector('[data-elem="goods-cart-wrapper"]');
        childrenArray = childrenArray.children;
        /*skip a usual event handler*/
        evt.preventDefault();
        /*if scrolling up*/
        if (evt.deltaY > 0) {
            /*if a cart has been decreased-*/
        ...
        /*assign a current position*/
        childrenArray[currentIndex].scrollIntoView(true);
    }
...
var useCase0001;
var scrBar;
function onScr0001 (evt) {
   console.log(evt.currentTarget.scrollHeight);
  evt.preventDefault();
    evt.currentTarget.scrollTop = 50;
}
